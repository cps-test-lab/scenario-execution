...
class FollowWaypointsFromFile(FollowWaypoints):
    ...
    one sequence per (condition, repetition) -- cannot write that sequence into the `.osc` or into
    parameter overrides without either becoming the data store for someone else's planner output.
    This reads it from disk instead, where the planner already wrote it.

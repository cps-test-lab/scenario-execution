...
class NamespaceAwareBasicNavigator(BasicNavigator):
    ...
    def __init__(self, node_name, namespace):
        ...
        super(BasicNavigator, self).__init__(
            node_name=node_name, namespace=namespace  # pylint: disable=bad-super-call
        ...
        self.clear_costmap_global_srv = self.create_client(ClearEntireCostmap, 'global_costmap/clear_entirely_global_costmap', callback_group=self.cb_group)
        self.clear_costmap_local_srv = self.create_client(ClearEntireCostmap, 'local_costmap/clear_entirely_local_costmap', callback_group=self.cb_group)
        self.get_costmap_global_srv = self.create_client(GetCostmap, 'global_costmap/get_costmap', callback_group=self.cb_group)
        self.get_costmap_local_srv = self.create_client(GetCostmap, 'local_costmap/get_costmap', callback_group=self.cb_group)

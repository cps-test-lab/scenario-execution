...
class GetEntityBounds(RosServiceCall):
    ...
    def check_response(self, msg):
        ...
        if not msg.result.result == Result.RESULT_OK:
            self.feedback_message = f"Failed to get entity bounds for {self.entity}: result {msg.result.result}, error_message {msg.result.error_message}"
            return False
        self.feedback_message = f"Retrieved bounds for entity {self.entity} successfully"
        return True

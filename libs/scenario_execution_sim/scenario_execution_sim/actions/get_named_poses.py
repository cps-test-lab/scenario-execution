...
class GetNamedPoses(RosServiceCall):
    ...
    def check_response(self, msg):
        ...
        if not msg.result.result == Result.RESULT_OK:
            self.feedback_message = f"Failed to get named poses: result {msg.result.result}, error_message {msg.result.error_message}"
            return False
        self.feedback_message = f"Retrieved {len(msg.poses)} named poses successfully"
        return True

...
class GetSpawnables(RosServiceCall):
    ...
    def check_response(self, msg):
        ...
        if not msg.result.result == Result.RESULT_OK:
            self.feedback_message = f"Failed to get spawnables: result {msg.result.result}, error_message {msg.result.error_message}"
            return False
        self.feedback_message = f"Retrieved {len(msg.spawnables)} spawnables successfully"
        return True

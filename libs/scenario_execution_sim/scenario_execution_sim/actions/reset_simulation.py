...
class ResetSimulation(RosServiceCall):
    ...
    def execute(self):   # pylint: disable=arguments-differ,arguments-renamed
        super().execute(data=f'{{ "scope": {self.scope} }}')
    ...
    def check_response(self, msg):
        ...
        if not msg.result.result == Result.RESULT_OK:
            self.feedback_message = f"Failed to reset simulation: result {msg.result.result}, error_message {msg.result.error_message}"
            return False
        self.feedback_message = f"Simulation reset successfully with scope {self.scope}"
        return True

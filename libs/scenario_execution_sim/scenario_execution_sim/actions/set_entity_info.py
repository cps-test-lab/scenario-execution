...
class SetEntityInfo(RosServiceCall):
    ...
    def execute(self):   # pylint: disable=arguments-differ,arguments-renamed
        super().execute(data=f'{{ "entity": "{self.entity}", "info": {self.info} }}')
    ...
    def check_response(self, msg):
        ...
        if not msg.result.result == Result.RESULT_OK:
            self.feedback_message = f"Failed to set entity info for {self.entity}: result {msg.result.result}, error_message {msg.result.error_message}"
            return False
        self.feedback_message = f"Set entity info for {self.entity} successfully"
        return True

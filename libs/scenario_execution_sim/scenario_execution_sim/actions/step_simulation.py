...
class StepSimulation(RosServiceCall):
    ...
    def execute(self):   # pylint: disable=arguments-differ,arguments-renamed
        super().execute(data=f'{{ "steps": {self.steps} }}')
    ...
    def check_response(self, msg):
        ...
        if not msg.result.result == Result.RESULT_OK:
            self.feedback_message = f"Failed to step simulation: result {msg.result.result}, error_message {msg.result.error_message}"
            return False
        self.feedback_message = f"Stepped simulation {self.steps} steps successfully"
        return True

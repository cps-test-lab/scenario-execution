...
class TestOSC2Parser(unittest.TestCase):
    ...
    def test_scenario_file_not_existing(self):
        scenario_execution = ScenarioExecution(debug=False, log_model=False, live_tree=False, scenario_file='UNKNOWN_FILE.sce', output_dir="")
    ...
    def test_scenario_file_wrong_extension(self):
        scenario_execution = ScenarioExecution(debug=False, log_model=False, live_tree=False, scenario_file='UNKNOWN_FILE', output_dir="")
        result = scenario_execution.parse()
        self.assertFalse(result)
        self.assertEqual(len(scenario_execution.results), 1)
        self.assertEqual(scenario_execution.results[0].result, False)
    ...
    def test_no_scenario(self):
        scenario_execution = ScenarioExecution(
            debug=False, log_model=False, live_tree=False, scenario_file=os.path.join(os.path.dirname(__file__), 'scenarios', 'no_scenario.osc'), output_dir=""
        )
        result = scenario_execution.parse()
        self.assertFalse(result)
        self.assertEqual(len(scenario_execution.results), 1)
